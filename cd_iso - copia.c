...
#include <debugnet.h>
static char *ISO_FILENAME = "ux0:data/SOMO00003/Heart Of The Alien (U).iso";
...
int read_file(const char *filename, void *out)
{
	int size, offset;
	char archive[256];
snprintf(archive, sizeof archive, "%s%s", "ux0:data/SOMO00003/", filename);
debugNetPrintf(DEBUG,"size %d",size);
...
debugNetPrintf(DEBUG,"use_iso %s",archive);
