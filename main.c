...
#define ip_server "192.168.0.198"
...
debugNetPrintf(DEBUG,"SDL_INIT_VIDEO");
	SDL_Init(SDL_INIT_VIDEO|SDL_INIT_AUDIO);
	debugNetPrintf(DEBUG,"atexit");
...
debugNetPrintf(DEBUG,"atexit");
	if (nosound_flag == 0)
	{
		...
		music_init();
		sound_init();
		debugNetPrintf(DEBUG,"sound_init");
	}
	if (render_init() < 0)
	{
...
void load_room_screen(int room, int index)
{
	...
	//SDL_SetSurfacePalette(_window, sdlpalette);
...
void check_events()
{
	...
	SDL_PollEvent(&event);
	debugNetPrintf(DEBUG,"event.jbutton.button %d\n", event.jbutton.button);
	debugNetPrintf(DEBUG,"event.jbutton.button %d\n", event.jbutton.button);
	#ifdef PSVITA
		...
		case SDL_JOYBUTTONDOWN:
			if (_joystick) {
				const int pressed = (event.jbutton.state == SDL_PRESSED);
				switch (event.jbutton.button) {
				case 0:
					{
						if (pressed) {
...
void run()
{
	 ...
	 */
	 debugNetPrintf(DEBUG,"init_tasks");
	init_tasks();
	...
	while (quit == 0)
	{
		...
		SDL_UpdateWindowSurfaceRects(_window,&srcrect,1);
