...
void draw_sprites()
{
	//debugNetPrintf(DEBUG,"draw_sprites");
...
void op_85()
{
	...
	loc_bf88:
	//debugNetPrintf(DEBUG,"NOT IMPLMENTED!\n");
	assert(0);
}
...
void collision_against_set()
{
	...
	//debugNetPrintf(DEBUG,"collision of sprite %d\n", entry_src);
	if (sprite_count == 0)
	{
		/* no sprites at all! */
		set_variable(2, 0);
...
void op_27()
{
	...
	if (d1 & 0x80)
	{
		...
		if (d1 == 0x13)
		{
			...
			//debugNetPrintf(DEBUG,"var[2] was set to 0x%x\n", d0);
		}
		else
		{	
			//debugNetPrintf(DEBUG,"loc_d1_ne_0x13: set_var(2, %d) d1=%d entry=%d\n", d1, entry, get_sprite_data_word(entry, d1));
...
void copy_tls_to_global()
{
	...
	while (count >= 0)
	{
		//debugNetPrintf(DEBUG,"copy tls-var %d into globalvar %d\n",  src_index, dst_index);
...
void copy_global_to_tls()
{
	...
	//debugNetPrintf(DEBUG,"copy %d from global to tls starting at src=%d, dst=%d\n", count + 1, src_index, dst_index);

	/* copy variables from variables to active-var-vector */
	while (count >= 0)
	{
		//debugNetPrintf(DEBUG,"copy globalvar %d into tls-var %d\n", src_index, dst_index);
...
void op_2a()
{
	...
	//debugNetPrintf(DEBUG,"sprite d0 = %d\n", d0);
	...
	//debugNetPrintf(DEBUG," 	comparing d0 with get_byte(a4), %d with %d\n", d0, get_byte(a4));
	if (d0 > get_byte(a4))
	{
		/* c3f8 */
		...
		//debugNetPrintf(DEBUG,"assigning d0, d2, d3 with %d %d %d\n", d0, d2, d3);
	...
	else
	{
		...
		//debugNetPrintf(DEBUG,"d3 loaded is %d(%x)\n", d3, d3);
	}

	//debugNetPrintf(DEBUG,"before d3=%d after =%d\n", d3, extw(d3));
	d3 = extw(d3); 
	d2 = extw(d2); 
	...
	//debugNetPrintf(DEBUG,"new sprites[which].x == %d\n", sprites[which].x);

	/* loc_c45c: */
	//debugNetPrintf(DEBUG,"before adding y=%d, d3=%d\n", sprites[which].y, d3);
	sprites[which].y += d3;
	//debugNetPrintf(DEBUG,"new sprites[which].y == %d (which=%d)\n", sprites[which].y, which);
...
void op_70()
{
	...
	//debugNetPrintf(DEBUG,"op70 d0=%d, d1=%d\n", d0, d1);
	...
	dummy = dummy - d1;
	sprites[entry].x = dummy;
	//debugNetPrintf(DEBUG,"loc_c0ec new sprite.x = %d\n", dummy);
	goto loc_mainloop;
	...
	d0 = d0 - d1;
	sprites[entry].y = d0;
	//debugNetPrintf(DEBUG,"loc_c190 new sprite.y = %d\n", d0);
	d1 = d1 >> 1;
	d3 = d3 - d1;
	d0 = get_byte(a4+4);
	sprites[entry].u7 = d0;
...
void new_task()
{
	...
	//debugNetPrintf(DEBUG,"*newtask* op_08 d0=%x d1=%x\n", d0, d1);
	...
	if (d2 != 0)
	{
		...
		while (d0 >= 0)
		{
			...
			//debugNetPrintf(DEBUG,"auxvars[%d][%d] == var[%d] (=0x%x)\n", a2/32, a2%32, d1, auxvars[a2]);
			a2++;
		}
		...
		while (d0 >= 0)
		{
			...
			//debugNetPrintf(DEBUG,"auxvars[%d][%d] == 0x%x\n", a2/32, a2%32, auxvars[a2]);
			a2++;
		}
...
int decode(int current_task, int start_pc)
{
	...
	while (leave == 0)
	{
		...
		mark_opcode(opcode);
		//debugNetPrintf(DEBUG,"(%d) task: %d: %04x 0x%02x: ", stack_ptr, current_task, pc-1, opcode);
		//debugNetPrintf(DEBUG,"(%d) task: %d: %04x 0x%02x: ", stack_ptr, current_task, pc-1, opcode);
	...
	//debugNetPrintf(DEBUG,"var[%d] = 0x%x\n", var1, imm16);
			//debugNetPrintf(DEBUG,"var[%d] = 0x%x\n", var1, imm16);
			break;
	...
	//debugNetPrintf(DEBUG,"var[%d] = var[%d] (=0x%x)\n", var1, var2, get_variable(var2));
			//debugNetPrintf(DEBUG,"var[%d] = var[%d] (=0x%x)\n", var1, var2, get_variable(var2));
			break;
...
//debugNetPrintf(DEBUG,"var[%d] += var[%d] (now 0x%x)\n", var1, var2, get_variable(var1));
			//debugNetPrintf(DEBUG,"var[%d] += var[%d] (now 0x%x)\n", var1, var2, get_variable(var1));
			break;
...
//debugNetPrintf(DEBUG,"var[%d] += 0x%x (now 0x%x)\n", var1, imm16, get_variable(var1));
			//debugNetPrintf(DEBUG,"var[%d] += 0x%x (now 0x%x)\n", var1, imm16, get_variable(var1));
			break;
			...
			push(pc);
			pc = imm16;
	//debugNetPrintf(DEBUG,"call 0x%x\n", imm16);
			//debugNetPrintf(DEBUG,"call 0x%x\n", imm16);
			break;
	
			case 0x05:
			pc = pop();
			//debugNetPrintf(DEBUG,"ret\n");
			//debugNetPrintf(DEBUG,"ret\n");
			break;
	
			case 0x06:
			//debugNetPrintf(DEBUG,"yield\n");
			//debugNetPrintf(DEBUG,"yield\n");
			leave = 1;
			break;
	...
	//debugNetPrintf(DEBUG,"dbf var[%d], goto 0x%x(now=0x%x)\n", var1, jmpto, imm16);
			//debugNetPrintf(DEBUG,"dbf var[%d], goto 0x%x(now=0x%x)\n", var1, jmpto, imm16);
			break;
			...
			if (compare_op & 0x80)
			{
				...
				//debugNetPrintf(DEBUG,"if var[%d] %s var[%d] goto 0x%x(lv=%x, rv=%x)\n", var1, cmpopar[compare_op & 0x07], var2, jmpto, imm16, imm16_2);
				//debugNetPrintf(DEBUG,"if var[%d] %s var[%d] goto 0x%x(lv=%x, rv=%x)\n", var1, cmpopar[compare_op & 0x07], var2, jmpto, imm16, imm16_2);
			}
			else if (compare_op & 0x40)
			{
				...
				jmpto = next_pc_word();
				//debugNetPrintf(DEBUG,"(imm16) if var[%d] %s %d goto 0x%x (lv=%x)\n", var1, cmpopar[compare_op & 0x07], imm16_2, jmpto, imm16);
				//debugNetPrintf(DEBUG,"(imm16) if var[%d] %s %d goto 0x%x (lv=%x)\n", var1, cmpopar[compare_op & 0x07], imm16_2, jmpto, imm16);
			}
			else
			{
				...
				imm16_2 = next_pc();
				jmpto = next_pc_word();
				//debugNetPrintf(DEBUG,"(imm8) if var[%d] %s %d goto 0x%x (lv=%x)\n", var1, cmpopar[compare_op & 0x07], imm16_2, jmpto, imm16);
			}
			...
			switch(compare_op & 0x07)
			{
				...
				default:
				//debugNetPrintf(DEBUG,"UNKNOWN COMPARE OPERATOR! %d\n", compare_op & 0x07);
				//debugNetPrintf(DEBUG,"UNKNOWN COMPARE OPERATOR! %d\n", compare_op & 0x07);
				assert(0);
				break;
			}
			...
			/* set palette imm8 */
			imm8 = next_pc();
	//debugNetPrintf(DEBUG,"set palette %d\n", imm8);
	...
	//debugNetPrintf(DEBUG,"select screen %d\n", imm8);
			select_screen(imm8);
			//debugNetPrintf(DEBUG,"select screen %d\n", imm8);
			break;
			...
			imm8 = next_pc();
			imm8_2 = next_pc();
	//debugNetPrintf(DEBUG,"fill_screen %d\n", imm8);
			fill_screen(imm8, imm8_2);
			//debugNetPrintf(DEBUG,"fill screen%d with color %d\n", imm8, imm8_2);
			break;
			...
			//debugNetPrintf(DEBUG,"screen%d <= screen%d\n", imm8_2, imm8);
			break;
	
			case 0x10:
			imm8 = next_pc();
			update_screen(imm8);
			//debugNetPrintf(DEBUG,"update screen %d\n", imm8);
			break;
			...
			leave = 1;
			pc = -1;
			//debugNetPrintf(DEBUG,"terminate task!\n");
			break;
			...
			//debugNetPrintf(DEBUG,"var[%d] -= var[%d]\n", var1, var2);
			...
			set_variable(var1, imm16);
			//debugNetPrintf(DEBUG,"var[%d] >>= %d (now 0x%x)\n", var1, imm8, imm16);
			break;

			case 0x18:
			//debugNetPrintf(DEBUG,"play sample");
			...
			/* load screen imm16 */
			imm16 = next_pc_word();
			//debugNetPrintf(DEBUG,"load screen %d (0x%x)\n", imm16, imm16);
			...
			case 0x1a:
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"play audio track %d\n", imm8);
			...
			else if (imm8 >= 100)
			{
				...
				//debugNetPrintf(DEBUG,"play single audio track %d\n", imm8);
			...
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"switch of variable %d over %d cases\n", var1, imm8);
			...
			//debugNetPrintf(DEBUG,"if var[%d] == 0 goto 0x%x\n", var1, jmpto);
			break;
			...
			//debugNetPrintf(DEBUG,"if var[%d] != 0x00 goto 0x%x(lv=0x%x)\n", var1, jmpto, get_variable(var1));
			break;

			case 0x21:
			imm8 = next_pc() % 32;
			//debugNetPrintf(DEBUG,"play death animation %d\n", imm8);
			...
			case 0x25:
			//debugNetPrintf(DEBUG,"add sprite\n");
			add_sprite();
			break;
			...
			case 0x28:
			//debugNetPrintf(DEBUG,"collision against set\n");
			...
			case 0x29:
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"remove sprite %d\n", imm8);
			remove_sprite(imm8);
			break;
			...
			//debugNetPrintf(DEBUG,"move sprite %d by (%d, %d)\n", imm16, x, y);
			...
			flip_sprite(imm16);
			//debugNetPrintf(DEBUG,"mirror/unmirror sprite %d\n", imm16);
			break;
			...
			mirror_sprite(imm16);
			//debugNetPrintf(DEBUG,"mirror sprite %d\n", imm16);
			break;

			case 0x33:
			imm16 = get_variable(next_pc());
			unmirror_sprite(imm16);
			//debugNetPrintf(DEBUG,"unmirror sprite %d\n", imm16);
			break;

			case 0x34:
			//debugNetPrintf(DEBUG,"d4=0\n");
			saved_d4 = 0;
			break;

			case 0x35:
			//debugNetPrintf(DEBUG,"d5=0\n");
			saved_d5 = 0;
			break;

			case 0x36:
			saved_d4 = next_pc_word();
			//debugNetPrintf(DEBUG,"saved_d4 = 0x%x\n", saved_d4);
			break;

			case 0x37:
			saved_d5 = next_pc_word();
			//debugNetPrintf(DEBUG,"saved_d5 = 0x%x\n", saved_d5);
			break;

			case 0x38:
			imm16 = next_pc_word();
			saved_d4 = saved_d4 + imm16;
			//debugNetPrintf(DEBUG,"d4 += %d (now %d)\n", imm16, saved_d4);
			break;

			case 0x39:
			imm16 = next_pc_word();
			saved_d5 = saved_d5 + imm16;
			//debugNetPrintf(DEBUG,"d5 += %d (now %d)\n", imm16, saved_d5);
			break;

			case 0x3a:
			imm16 = next_pc_word();
			saved_d4 &= imm16;
			//debugNetPrintf(DEBUG,"d4 &= 0x%x (now=%x)\n", imm16, saved_d4);
			break;

			case 0x3b:
			imm16 = next_pc_word();
			saved_d5 &= imm16;
			//debugNetPrintf(DEBUG,"d5 &= 0x%x (now=%x)\n", imm16, saved_d5);
			break;

			case 0x3e:
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"d4 <<= %d\n", imm8);
			saved_d4 <<= imm8;
			break;

			case 0x3f:
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"d5 <<= %d\n", imm8);
			saved_d5 <<= imm8;
			break;
			...
			//debugNetPrintf(DEBUG,"saved_d4 >>= %d (now 0x%x)\n", imm8, saved_d4);
			break;

			case 0x41:
			imm8 = next_pc();
			saved_d5 = extl(saved_d5) >> imm8;
			//debugNetPrintf(DEBUG,"saved_d5 >>= %d (now 0x%x)\n", imm8, saved_d5);
			break;

			case 0x42:
			saved_d4 = saved_d5;
			//debugNetPrintf(DEBUG,"saved_d4 = saved_d5\n");
			break;

			case 0x43:
			saved_d5 = saved_d4;
			//debugNetPrintf(DEBUG,"d5 = d4\n");
			break;

			case 0x4a:
			var1 = next_pc();
			saved_d4 = get_variable(var1);
			//debugNetPrintf(DEBUG,"saved_d4 = var[%d] (=0x%x)\n", var1, saved_d4);
			break;

			case 0x4b:
			var1 = next_pc();
			saved_d5 = get_variable(var1);
			//debugNetPrintf(DEBUG,"saved_d5 = var[%d] (=0x%x)\n", var1, saved_d5);
			break;
			
			case 0x4c:
			var1 = next_pc();
			set_variable(var1, saved_d4);
			//debugNetPrintf(DEBUG,"var[%d] = saved_d4 (=0x%x)\n", var1, saved_d4);
			break;

			case 0x4d:
			var1 = next_pc();
			set_variable(var1, saved_d5);
			//debugNetPrintf(DEBUG,"var[%d] = saved_d5 (=0x%x)\n", var1, saved_d5);
			break;
			...
			//debugNetPrintf(DEBUG,"if d4 %s 0x%04x goto 0x%x(lv=%x)\n", cmpopar[compare_op], imm16, jmpto, saved_d4);
			...
			//debugNetPrintf(DEBUG,"var[%d] = 0\n", var1);
			break;

			case 0x5c:
			var1 = next_pc();
			set_variable(var1, get_variable(var1) + saved_d4);
			//debugNetPrintf(DEBUG,"var[%d] += d4 (now 0x%04x)\n", var1, get_variable(var1));
			break;

			case 0x5d:
			var1 = next_pc();
			set_variable(var1, get_variable(var1) + saved_d5);
			//debugNetPrintf(DEBUG,"var[%d] += d5 (now 0x%04x)\n", var1, get_variable(var1));
			break;

			case 0x5e:
			var1 = next_pc();         
			saved_d4 = saved_d4 - get_variable(var1);
			//debugNetPrintf(DEBUG,"d4 -= var[%d] (now 0x%x)\n", var1, saved_d4);
			break;

			case 0x5f:
			var1 = next_pc();
			saved_d5 = saved_d5 - get_variable(var1);
			//debugNetPrintf(DEBUG,"d5 -= var[%d] (now 0x%x)\n", var1, saved_d5);
			break;
			...
			set_variable(var1, imm16);
			//debugNetPrintf(DEBUG,"var[%d] -= d4 (now 0x%x)\n", var1, imm16);
			break;
			...
			//debugNetPrintf(DEBUG,"if d4 %s var[%d] goto 0x%x(lv=%x, rv=%x)\n", cmpopar[compare_op], var1, jmpto, saved_d4, imm16);
			...
			switch(compare_op)
			{
				...
				default:
				//debugNetPrintf(DEBUG,"UNKNOWN COMPARE OP %d\n", compare_op);
				assert(0);
			}
			break;
			...
			//debugNetPrintf(DEBUG,"if d5 %s var[%d] goto 0x%x(lv=%x, rv=%x)\n", cmpopar[compare_op], var1, jmpto, saved_d4, imm16);
			...
			//debugNetPrintf(DEBUG,"switch of d4 with %d cases (d4 == 0x%x)\n", imm8, saved_d4);
			while (imm8 > 0)
			{
				imm16 = next_pc_word();
				jmpto = next_pc_word();
				//debugNetPrintf(DEBUG,"on 0x%x --> goto 0x%x\n", imm16, jmpto);
			...
			case 0x6a:
			jmpto = next_pc_word();
			//debugNetPrintf(DEBUG,"if d4 == 0 goto 0x%x (d4=0x%x)\n", jmpto, saved_d4);
			if (saved_d4 == 0)
			{
				pc = jmpto;	
			}
			...
			case 0x6b:
			jmpto = next_pc_word();
			//debugNetPrintf(DEBUG,"if d5 == 0 goto 0x%x (d5=0x%x)\n", jmpto, saved_d5);
			if (saved_d5 == 0)
			{
				pc = jmpto;	
			}
			...
			//debugNetPrintf(DEBUG,"d4 = %d (0x%x)\n", saved_d4, saved_d4);
			break;


			case 0x6f:
			saved_d5 = extw(next_pc());
			//debugNetPrintf(DEBUG,"d5 = %d (0x%x)\n", saved_d5, saved_d5);
			break;
			...
			case 0x73:
			saved_d4++;
			//debugNetPrintf(DEBUG,"d4++ (now 0x%x)\n", saved_d4);
			break;

			case 0x74:
			saved_d5++;
			//debugNetPrintf(DEBUG,"d5++ (now 0x%x)\n", saved_d5);
			break;
			...
			//debugNetPrintf(DEBUG,"var[%d] asl %d\n", var1, imm8); 
			break;
			...
			//debugNetPrintf(DEBUG,"asr %d var[%d] (now 0x%x)\n", imm8, var1, imm16);
			break;

			case 0x7d:
			imm16 = get_variable(next_pc());
			is_last_frame_in_animation(imm16);
			//debugNetPrintf(DEBUG,"check if current frame exceeded frame count for sprite %d\n", imm16);
			break;
			...
			//debugNetPrintf(DEBUG,"shift sprite %d delta_x=%d (%x)\n", imm16, x, x);
			break;
			...
			//debugNetPrintf(DEBUG,"shift sprite %d delta_y=%d (%x)\n", imm16, y, y);
			break;

			case 0x80:
			toggle_aux(0);
			//debugNetPrintf(DEBUG,"using_aux = 0\n");
			break;

			case 0x81:
			toggle_aux(1);
			//debugNetPrintf(DEBUG,"using_aux = 1\n");
			break;

			case 0x82:                
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"breakpoint %d\n", imm8);
			break;
			...
			//debugNetPrintf(DEBUG,"indirect jump to 0x%x\n", pc);
			break; 
			...
			//debugNetPrintf(DEBUG,"read byte %d from array at 0x%x into var[%d] (%04x)\n", get_variable(var2), imm16, var1, extw(get_byte(script_ptr + imm16_2)));
			...
			//debugNetPrintf(DEBUG,"set palette var[%d] (%d)\n", var1, imm16);
			break;

			case 0x8c:
			imm8 = next_pc();
			//debugNetPrintf(DEBUG,"scroll reg = %d\n", extw(imm8));
			...
			*/
			...
			default:
			//debugNetPrintf(DEBUG,"opcode 0x%x unknown\n", opcode);
			panic("unknown!");
			break;
		}
	}
